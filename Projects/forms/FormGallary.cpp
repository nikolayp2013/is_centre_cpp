	...
	: TForm(Owner)
{
	...
	MyConnection1->Server = IniSer->ReadString("ServerConf","HostName","");
	MyConnection1->Username = IniSer->ReadString("ServerConf","UserName","");
	MyConnection1->Password = IniSer->ReadString("ServerConf","PassWord","");
	...
	TabPricelist_Datasource_Main_local = new TDataSource(NULL);
	TabPricelist_Datasource_Main_local->DataSet=MainTableLocal;
	TabPricelist_Datasource_Main_local->DataSet->Active=TRUE;

	Table_pics_Datasource = new TDataSource(NULL);
	Table_pics_Datasource->DataSet=Table_pics;

	this->DBMainGrid->DataSource= TabPricelist_Datasource_Main_local;
...
void __fastcall TFormGall::But_addClick(TObject *Sender)
{
	...
	Table_pics_Datasource->DataSet->FieldByName("IDPRICE")->Value =
		TabPricelist_Datasource_Main_local->DataSet->FieldByName("ID")->Value;
	Table_pics_Datasource->DataSet->FieldByName("IDPARENT")->AsString =
		TabPricelist_Datasource_Main_local->DataSet->FieldByName("GUID")->AsString;
