...
TFormEditRoomsSell *FormEditRoomsSell;
...
void __fastcall TFormEditRoomsSell::FormCreate(TObject *Sender)
{
...
//---------------------------------------------------------------------------
__fastcall TFormEditRoomsSell::TFormEditRoomsSell(TComponent* Owner)
			: TForm(Owner)
{
...
//---------------------------------------------------------------------------
void __fastcall TFormEditRoomsSell::ProcRefreshClient(TObject *Sender)
{
...
void __fastcall TFormEditRoomsSell::FormShow(TObject *Sender)
{
	//Район
	...
	//Указываем цвет элемента
	Streets_DBEdit->Color=(TColor)RGB(255,220,220);
	Level_DBEdit->Color=(TColor)RGB(255,220,220);
	Levels_DBEdit->Color=(TColor)RGB(255,220,220);
	Typehouse_DBLookComb->Color=(TColor)RGB(255,220,220);
	Stuffwall_DBLookComb->Color=(TColor)RGB(255,220,220);
	Rooms_DBEdit->Color=(TColor)RGB(255,220,220);
	Totalarea_DBEdit->Color=(TColor)RGB(255,220,220);
	Livearea_DBEdit->Color=(TColor)RGB(255,220,220);
	Kitchenarea_DBEdit->Color=(TColor)RGB(255,220,220);
}
//---------------------------------------------------------------------------


void __fastcall TFormEditRoomsSell::PageControl1DrawTab(TCustomTabControl *Control, int TabIndex,
...
void __fastcall TFormEditRoomsSell::Streets_DBEditKeyUp(TObject *Sender, WORD &Key, TShiftState Shift)
{
...
void __fastcall TFormEditRoomsSell::Streets_DBLookupListBoxClick(TObject *Sender)
{
...
void __fastcall TFormEditRoomsSell::Streets_DBEditEnter(TObject *Sender)
{
...
void __fastcall TFormEditRoomsSell::Streets_DBEditExit(TObject *Sender)
{
...
void __fastcall TFormEditRoomsSell::Streets_DBLookupListBoxKeyDown(TObject *Sender, WORD &Key,
          TShiftState Shift)
{
	 if (Key==13) {
		if (Streets_DBLookupListBox->Top==98) {
...
void __fastcall TFormEditRoomsSell::Streets_DBLookupListBoxMouseUp(TObject *Sender, TMouseButton Button,
...
void __fastcall TFormEditRoomsSell::Streets_DBLookupListBoxExit(TObject *Sender)
{
	if (Streets_DBLookupListBox->Top==98) {
		Streets_DBEdit->DataSource->DataSet->FieldByName("STREET")->Value=
			Streets_DBLookupListBox->KeyValue;
...
void __fastcall TFormEditRoomsSell::Cross_DBEditEnter(TObject *Sender)
{
...
void __fastcall TFormEditRoomsSell::Cross_DBEditKeyDown(TObject *Sender, WORD &Key, TShiftState Shift)
{
...
void __fastcall TFormEditRoomsSell::Cross_DBEditExit(TObject *Sender)
{
	if (!Streets_DBLookupListBox->Focused()){
		if (Cross_DBEdit->Text.Length()!=0) {
			Streets_DBEdit->DataSource->DataSet->FieldByName("CRSTREET")->Value=
...
void __fastcall TFormEditRoomsSell::OnApplicationMessage(TMsg & Msg, bool & Handled)
{
...
void __fastcall TFormEditRoomsSell::SpeedButton2Click(TObject *Sender)
{
	// Кнопка отмена
	if (swinsert==1) {
    	// Если регистрация
...
void __fastcall TFormEditRoomsSell::Button6Click(TObject *Sender)
{
	//FormGall = new TFormGall(FormEdit,MainForm->MainConn,MainTransaction,TabPricelist_Datasource_Main);
	//FormGall->ShowModal();
	//delete FormGall;
}
//---------------------------------------------------------------------------

void __fastcall TFormEditRoomsSell::Button4Click(TObject *Sender)
{
...
void __fastcall TFormEditRoomsSell::SpeedButton1Click(TObject *Sender)
{
...
void __fastcall TFormEditRoomsSell::SpeedButton3Click(TObject *Sender)
{
    MainForm->TabPricelist_Datasource_Main->DataSet->FieldByName("CLIENT")->AsInteger=0;
	// Вызов процедуры обновления сведений о клиенте
	ProcRefreshClient(this);
...
void __fastcall TFormEditRoomsSell::Button5Click(TObject *Sender)
{
	// Кнопка назад
...
void __fastcall TFormEditRoomsSell::Button3Click(TObject *Sender)
{
	// Кнопка вперед
...
void __fastcall TFormEditRoomsSell::ProcSave(TObject *Sender)
{
    //Фиксация изменений
...
void __fastcall TFormEditRoomsSell::RefreshAgent(TObject *Sender)
{
...
void __fastcall TFormEditRoomsSell::Agent_ComboBoxClick(TObject *Sender)
{
	 Agent_datasource->DataSet->Active=true;
	 Agent_datasource->DataSet->First();
...
void __fastcall TFormEditRoomsSell::Button1Click(TObject *Sender)
{
...
void __fastcall TFormEditRoomsSell::Button2Click(TObject *Sender)
{
...
void __fastcall TFormEditRoomsSell::Button7Click(TObject *Sender)
{
    // Кнопка сохранить
	String ds = "";
	if (Streets_DBEdit->Text.IsEmpty()) {
		ds = ds+"- не указана улица \n";
		//break;
	}
	if (Levels_DBEdit->Text.IsEmpty()) {
		ds = ds+"- не указана этажность дома \n";
	}
	if (Price_DBEdit->Text.IsEmpty() || Price_DBEdit->Text=="0") {
		ds = ds+"- не указана цена \n";
	}
	if (Totalarea_DBEdit->Text.IsEmpty() || Livearea_DBEdit->Text.IsEmpty() || Kitchenarea_DBEdit->Text.IsEmpty()) {
		ds = ds+"- не указаны площади \n";
	}
	if (Typehouse_DBLookComb->Text.IsEmpty()) {
		ds = ds+"- не указан тип дома \n";
	}
	if (Stuffwall_DBLookComb->Text.IsEmpty()) {
		ds = ds+"- не указан материал стен \n";
	}
	if (Agent_ComboBox->Text.IsEmpty()) {
		ds = ds+"- не указан агент \n";
	}
	if (!ds.IsEmpty()) {
		 MessageBox(NULL,ds.c_str(),"Проверка заполнения",MB_OK | MB_TOPMOST | MB_ICONSTOP);
	} else {
