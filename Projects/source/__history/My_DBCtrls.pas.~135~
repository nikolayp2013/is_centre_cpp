...
{ TMyDBComboBox }
  ...
  TMyDBComboBox = class(TCustomComboBox)
  private
    FDataLink: TFieldDataLink;
    FPaintControl: TPaintControl;
    procedure DataChange(Sender: TObject);
...
{ TDBImage }
  ...
  TMyDBImage = class(TCustomControl)
  private
    FDataLink: TFieldDataLink;
    FPicture: TPicture;
...
{ TMyDBNavigator }
  ...
  TMyDBNavigator = class (TCustomPanel)
  private
...
{ TNavDataLink }
  ...
  private
    FNavigator: TMyDBNavigator;
  protected
    procedure EditingChanged; override;
    procedure DataSetChanged; override;
    procedure ActiveChanged; override;
  public
    constructor Create(ANav: TMyDBNavigator);
    destructor Destroy; override;
  end;
...
{ TMyDBLookupListBox }
  ...
  TMyDBLookupListBox = class(TDBLookupControl)
  private
...
{ TMyDBLookupComboBox }
  ...
  published
    ...
    property DataList: TPopupDataList read FDataList write FDataList;
    property OnContextPopup;
    property OnDragDrop;
    property OnDragOver;
...
const
  ...
  FieldAlignmentTbl: array[TFieldType] of Boolean= (
    ...
    False, False, False);        { ftConnection, ftParams, ftStream}
...
{ AField is needed because TMyDBLookupComboBox, for its combobox, uses FListField
...
function TMyDBEdit.GetDataField: WideString;
begin
  Result := FDataLink.FieldName;
end;

procedure TMyDBEdit.SetDataField(const Value: WideString);
begin
...
function TMyDBCheckBox.GetDataField: WideString;
begin
  Result := FDataLink.FieldName;
end;

procedure TMyDBCheckBox.SetDataField(const Value: WideString);
begin
  FDataLink.FieldName := Value;
end;
...
procedure TMyDBComboBox.Loaded;
begin
  inherited Loaded;
  if (csDesigning in ComponentState) then DataChange(Self);
end;

procedure TMyDBComboBox.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
  inherited Notification(AComponent, Operation);
  if (Operation = opRemove) and (FDataLink <> nil) and
...
procedure TMyDBComboBox.CreateWnd;
begin
  inherited CreateWnd;
  SetEditReadOnly;
end;

procedure TMyDBComboBox.DataChange(Sender: TObject);
begin
...
procedure TMyDBComboBox.UpdateData(Sender: TObject);
begin
  FDataLink.Field.Text := GetComboText;
end;

procedure TMyDBComboBox.SetComboText(const Value: string);
var
  I: Integer;
  Redraw: Boolean;
begin
...
function TMyDBComboBox.GetComboText: string;
var
  I: Integer;
begin
...
procedure TMyDBComboBox.Change;
begin
  FDataLink.Edit;
  inherited Change;
  FDataLink.Modified;
...
procedure TMyDBComboBox.DropDown;
begin
  inherited DropDown;
end;

function TMyDBComboBox.GetDataSource: TDataSource;
begin
  Result := FDataLink.DataSource;
end;

procedure TMyDBComboBox.SetDataSource(Value: TDataSource);
begin
  if not (FDataLink.DataSourceFixed and (csLoading in ComponentState)) then
    FDataLink.DataSource := Value;
  if Value <> nil then Value.FreeNotification(Self);
...
function TMyDBComboBox.GetDataField: WideString;
begin
  Result := FDataLink.FieldName;
end;

procedure TMyDBComboBox.SetDataField(const Value: WideString);
begin
  FDataLink.FieldName := Value;
end;

function TMyDBComboBox.GetReadOnly: Boolean;
begin
  Result := FDataLink.ReadOnly;
end;

procedure TMyDBComboBox.SetReadOnly(Value: Boolean);
begin
  FDataLink.ReadOnly := Value;
end;

function TMyDBComboBox.GetField: TField;
begin
  Result := FDataLink.Field;
end;

procedure TMyDBComboBox.KeyDown(var Key: Word; Shift: TShiftState);
begin
  inherited KeyDown(Key, Shift);
...
procedure TMyDBComboBox.KeyPress(var Key: Char);
begin
  inherited KeyPress(Key);
  if (Key >= #32) and (FDataLink.Field <> nil) and
    not FDataLink.Field.IsValidChar(Key) then
...
procedure TMyDBComboBox.EditingChange(Sender: TObject);
begin
  SetEditReadOnly;
end;

procedure TMyDBComboBox.SetEditReadOnly;
begin
  if (Style in [csDropDown, csSimple]) and HandleAllocated then
    SendMessage(EditHandle, EM_SETREADONLY, Ord(not FDataLink.Editing), 0);
end;

procedure TMyDBComboBox.WndProc(var Message: TMessage);
begin
  if not (csDesigning in ComponentState) then
    case Message.Msg of
      WM_COMMAND:
...
procedure TMyDBComboBox.ComboWndProc(var Message: TMessage; ComboWnd: HWnd;
  ComboProc: Pointer);
begin
  if not (csDesigning in ComponentState) then
    case Message.Msg of
...
procedure TMyDBComboBox.CMEnter(var Message: TCMEnter);
begin
  inherited;
  if FDataLink.CanModify then
...
procedure TMyDBComboBox.CMExit(var Message: TCMExit);
begin
  try
    FDataLink.UpdateRecord;
  except
...
procedure TMyDBComboBox.WMPaint(var Message: TWMPaint);
var
  S: string;
  R: TRect;
  P: TPoint;
...
procedure TMyDBComboBox.SetItems(const Value: TStrings);
begin
  inherited SetItems(Value);
  DataChange(Self);
end;

procedure TMyDBComboBox.SetStyle(Value: TComboboxStyle);
begin
...
function TMyDBComboBox.UseRightToLeftAlignment: Boolean;
begin
  Result := DBUseRightToLeftAlignment(Self, Field);
end;

procedure TMyDBComboBox.CMGetDatalink(var Message: TMessage);
begin
  Message.Result := Integer(FDataLink);
end;

function TMyDBComboBox.ExecuteAction(Action: TBasicAction): Boolean;
begin
  Result := inherited ExecuteAction(Action) or (FDataLink <> nil) and
    FDataLink.ExecuteAction(Action);
end;

function TMyDBComboBox.UpdateAction(Action: TBasicAction): Boolean;
begin
  Result := inherited UpdateAction(Action) or (FDataLink <> nil) and
    FDataLink.UpdateAction(Action);
end;
...
procedure TMyDBMemo.Loaded;
begin
  inherited Loaded;
  if (csDesigning in ComponentState) then DataChange(Self);
end;

procedure TMyDBMemo.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
  inherited Notification(AComponent, Operation);
  if (Operation = opRemove) and (FDataLink <> nil) and
...
function TMyDBMemo.UseRightToLeftAlignment: Boolean;
begin
  Result := DBUseRightToLeftAlignment(Self, Field);
end;

procedure TMyDBMemo.KeyDown(var Key: Word; Shift: TShiftState);
begin
  inherited KeyDown(Key, Shift);
  if FMemoLoaded then
  begin
...
procedure TMyDBMemo.KeyPress(var Key: Char);
begin
  inherited KeyPress(Key);
  if FMemoLoaded then
  begin
...
function TMyDBMemo.GetDataSource: TDataSource;
begin
  Result := FDataLink.DataSource;
end;

procedure TMyDBMemo.SetDataSource(Value: TDataSource);
begin
  if not (FDataLink.DataSourceFixed and (csLoading in ComponentState)) then
    FDataLink.DataSource := Value;
  if Value <> nil then Value.FreeNotification(Self);
...
function TMyDBMemo.GetDataField: Widestring;
begin
  Result := FDataLink.FieldName;
end;

procedure TMyDBMemo.SetDataField(const Value: WideString);
begin
  FDataLink.FieldName := Value;
end;

function TMyDBMemo.GetReadOnly: Boolean;
begin
  Result := FDataLink.ReadOnly;
end;

procedure TMyDBMemo.SetReadOnly(Value: Boolean);
begin
  FDataLink.ReadOnly := Value;
end;

function TMyDBMemo.GetField: TField;
begin
  Result := FDataLink.Field;
end;

procedure TMyDBMemo.LoadMemo;
begin
...
procedure TMyDBMemo.DataChange(Sender: TObject);
begin
  if FDataLink.Field <> nil then
...
procedure TMyDBMemo.EditingChange(Sender: TObject);
begin
  inherited ReadOnly := not (FDataLink.Editing and FMemoLoaded);
end;

procedure TMyDBMemo.UpdateData(Sender: TObject);
begin
  FDataLink.Field.Text := Text;
end;

procedure TMyDBMemo.SetFocused(Value: Boolean);
begin
  if FFocused <> Value then
  begin
    FFocused := Value;
...
procedure TMyDBMemo.WndProc(var Message: TMessage);
begin
  with Message do
    if (Msg = WM_CREATE) or (Msg = WM_WINDOWPOSCHANGED) or
...
procedure TMyDBMemo.CMEnter(var Message: TCMEnter);
begin
  SetFocused(True);
  inherited;
  if FDataLink.CanModify then
...
procedure TMyDBMemo.CMExit(var Message: TCMExit);
begin
  try
    FDataLink.UpdateRecord;
  except
...
procedure TMyDBMemo.SetAutoDisplay(Value: Boolean);
begin
...
procedure TMyDBMemo.WMLButtonDblClk(var Message: TWMLButtonDblClk);
begin
  if not FMemoLoaded then LoadMemo else inherited;
end;

procedure TMyDBMemo.WMCut(var Message: TMessage);
begin
  FDataLink.Edit;
  inherited;
end;

procedure TMyDBMemo.WMUndo(var Message: TMessage);
begin
  FDataLink.Edit;
  inherited;
end;

procedure TMyDBMemo.WMPaste(var Message: TMessage);
begin
  FDataLink.Edit;
  inherited;
end;

procedure TMyDBMemo.CMGetDataLink(var Message: TMessage);
begin
  Message.Result := Integer(FDataLink);
end;

procedure TMyDBMemo.WMPaint(var Message: TWMPaint);
var
  S: string;
begin
  if not (csPaintCopy in ControlState) then inherited else
...
function TMyDBMemo.ExecuteAction(Action: TBasicAction): Boolean;
begin
  Result := inherited ExecuteAction(Action) or (FDataLink <> nil) and
    FDataLink.ExecuteAction(Action);
end;

function TMyDBMemo.UpdateAction(Action: TBasicAction): Boolean;
begin
  Result := inherited UpdateAction(Action) or (FDataLink <> nil) and
    FDataLink.UpdateAction(Action);
end;
...
function TMyDBImage.DestRect(APicture: TPicture): TRect;
var
  w, h, cw, ch: Integer;
  xyaspect: Double;
begin
...
function TMyDBImage.GetDataSource: TDataSource;
begin
  Result := FDataLink.DataSource;
end;

procedure TMyDBImage.SetDataSource(Value: TDataSource);
begin
  if not (FDataLink.DataSourceFixed and (csLoading in ComponentState)) then
    FDataLink.DataSource := Value;
  if Value <> nil then Value.FreeNotification(Self);
...
function TMyDBImage.GetDataField: WideString;
begin
  Result := FDataLink.FieldName;
end;

procedure TMyDBImage.SetDataField(const Value: WideString);
begin
  FDataLink.FieldName := Value;
end;

function TMyDBImage.GetReadOnly: Boolean;
begin
  Result := FDataLink.ReadOnly;
end;

procedure TMyDBImage.SetReadOnly(Value: Boolean);
begin
  FDataLink.ReadOnly := Value;
end;

function TMyDBImage.GetField: TField;
begin
  Result := FDataLink.Field;
end;

function TMyDBImage.GetPalette: HPALETTE;
begin
  Result := 0;
...
procedure TMyDBImage.SetAutoDisplay(Value: Boolean);
begin
  if FAutoDisplay <> Value then
  begin
    FAutoDisplay := Value;
...
procedure TMyDBImage.SetBorderStyle(Value: TBorderStyle);
begin
...
procedure TMyDBImage.SetCenter(Value: Boolean);
begin
...
procedure TMyDBImage.SetPicture(Value: TPicture);
begin
  FPicture.Assign(Value);
end;

procedure TMyDBImage.SetProportional(Value: Boolean);
begin
...
procedure TMyDBImage.SetStretch(Value: Boolean);
begin
...
procedure TMyDBImage.PictureChanged(Sender: TObject);
begin
...
procedure TMyDBImage.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
  inherited Notification(AComponent, Operation);
  if (Operation = opRemove) and (FDataLink <> nil) and
...
procedure TMyDBImage.LoadPicture;
begin
...
procedure TMyDBImage.UpdateData(Sender: TObject);
begin
...
procedure TMyDBImage.CopyToClipboard;
begin
  if Picture.Graphic <> nil then Clipboard.Assign(Picture);
end;

procedure TMyDBImage.CutToClipboard;
begin
...
procedure TMyDBImage.PasteFromClipboard;
begin
...
procedure TMyDBImage.KeyDown(var Key: Word; Shift: TShiftState);
begin
  inherited KeyDown(Key, Shift);
  case Key of
    VK_INSERT:
...
procedure TMyDBImage.KeyPress(var Key: Char);
begin
  inherited KeyPress(Key);
  case Key of
    ^X: CutToClipBoard;
...
procedure TMyDBImage.CMGetDataLink(var Message: TMessage);
begin
  Message.Result := Integer(FDataLink);
end;

procedure TMyDBImage.CMEnter(var Message: TCMEnter);
begin
  Invalidate; { Draw the focus marker }
  inherited;
end;

procedure TMyDBImage.CMExit(var Message: TCMExit);
begin
  try
...
procedure TMyDBImage.CMTextChanged(var Message: TMessage);
begin
  inherited;
  if not FPictureLoaded then Invalidate;
end;

procedure TMyDBImage.WMLButtonDown(var Message: TWMLButtonDown);
begin
  if TabStop and CanFocus then SetFocus;
  inherited;
end;

procedure TMyDBImage.WMLButtonDblClk(var Message: TWMLButtonDblClk);
begin
  LoadPicture;
  inherited;
end;

procedure TMyDBImage.WMCut(var Message: TMessage);
begin
  CutToClipboard;
end;

procedure TMyDBImage.WMCopy(var Message: TMessage);
begin
  CopyToClipboard;
end;

procedure TMyDBImage.WMPaste(var Message: TMessage);
begin
  PasteFromClipboard;
end;

procedure TMyDBImage.WMSize(var Message: TMessage);
begin
  inherited;
  Invalidate;
end;

function TMyDBImage.ExecuteAction(Action: TBasicAction): Boolean;
begin
  Result := inherited ExecuteAction(Action) or (FDataLink <> nil) and
    FDataLink.ExecuteAction(Action);
end;

function TMyDBImage.UpdateAction(Action: TBasicAction): Boolean;
begin
  Result := inherited UpdateAction(Action) or (FDataLink <> nil) and
    FDataLink.UpdateAction(Action);
end;
...
procedure TMyDBNavigator.InitButtons;
var
  I: TNavigateBtn;
  Btn: TNavButton;
  X: Integer;
...
procedure TMyDBNavigator.InitHints;
var
  I: Integer;
  J: TNavigateBtn;
begin
...
procedure TMyDBNavigator.HintsChanged(Sender: TObject);
begin
  InitHints;
end;

procedure TMyDBNavigator.SetFlat(Value: Boolean);
var
  I: TNavigateBtn;
begin
  if FFlat <> Value then
...
procedure TMyDBNavigator.SetHints(Value: TStrings);
begin
...
function TMyDBNavigator.GetHints: TStrings;
begin
...
procedure TMyDBNavigator.GetChildren(Proc: TGetChildProc; Root: TComponent);
begin
end;

procedure TMyDBNavigator.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
  inherited Notification(AComponent, Operation);
  if (Operation = opRemove) and (FDataLink <> nil) and
...
procedure TMyDBNavigator.SetVisible(Value: TNavButtonSet);
var
  I: TNavigateBtn;
  W, H: Integer;
begin
...
procedure TMyDBNavigator.CalcMinSize(var W, H: Integer);
var
  Count: Integer;
  I: TNavigateBtn;
begin
...
procedure TMyDBNavigator.SetSize(var W: Integer; var H: Integer);
var
  Count: Integer;
  I: TNavigateBtn;
...
procedure TMyDBNavigator.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
var
  W, H: Integer;
begin
  W := AWidth;
...
procedure TMyDBNavigator.WMSize(var Message: TWMSize);
var
  W, H: Integer;
begin
  inherited;
...
procedure TMyDBNavigator.WMWindowPosChanging(var Message: TWMWindowPosChanging);
begin
  inherited;
...
procedure TMyDBNavigator.ClickHandler(Sender: TObject);
begin
  BtnClick (TNavButton (Sender).Index);
end;

procedure TMyDBNavigator.BtnMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
...
procedure TMyDBNavigator.WMSetFocus(var Message: TWMSetFocus);
begin
  Buttons[FocusedButton].Invalidate;
end;

procedure TMyDBNavigator.WMKillFocus(var Message: TWMKillFocus);
begin
  Buttons[FocusedButton].Invalidate;
end;

procedure TMyDBNavigator.KeyDown(var Key: Word; Shift: TShiftState);
var
...
procedure TMyDBNavigator.WMGetDlgCode(var Message: TWMGetDlgCode);
begin
  Message.Result := DLGC_WANTARROWS;
end;

procedure TMyDBNavigator.DataChanged;
var
...
procedure TMyDBNavigator.EditingChanged;
var
  CanModify: Boolean;
begin
...
procedure TMyDBNavigator.ActiveChanged;
var
  I: TNavigateBtn;
begin
...
procedure TMyDBNavigator.CMEnabledChanged(var Message: TMessage);
begin
  inherited;
...
procedure TMyDBNavigator.SetDataSource(Value: TDataSource);
begin
  FDataLink.DataSource := Value;
  if not (csLoading in ComponentState) then
    ActiveChanged;
...
function TMyDBNavigator.GetDataSource: TDataSource;
begin
  Result := FDataLink.DataSource;
end;

procedure TMyDBNavigator.Loaded;
var
  W, H: Integer;
begin
  inherited Loaded;
...
begin
  ...
  if (GetFocus = Parent.Handle) and
     (FIndex = TMyDBNavigator (Parent).FocusedButton) then
  begin
...
constructor TNavDataLink.Create(ANav: TMyDBNavigator);
begin
  inherited Create;
  FNavigator := ANav;
  VisualControl := True;
...
procedure TMyDBLookupListBox.WndProc(var Message: TMessage);
begin
  if not (csDesigning in ComponentState) then
    case Message.Msg of
      WM_VSCROLL or WM_HSCROLL:
        if (Message.wParam<>SB_ENDSCROLL) then
          begin
            if (loword(Message.wParam) = SB_THUMBTRACK) then
              begin
                ListLink.DataSet.MoveBy(HIWORD(Message.wParam)-ListLink.DataSet.RecNo);
              end;
            if (loword(Message.wParam) = SB_THUMBPOSITION) then
              begin
			      	  SetScrollPos(Handle, SB_VERT,HIWORD(Message.wParam),False);
              end;
          end;
  end;
  inherited WndProc(Message);
...
begin
 ...
 begin
    if ListLink.DataSet.RecordCount <> FRowCount then
        begin
            Max := ListLink.DataSet.RecordCount; // изменение, было 4
            if not ListLink.DataSet.BOF then
              if not ListLink.DataSet.EOF then Pos := ListLink.DataSet.RecNo  else Pos := ListLink.DataSet.RecordCount;
        end;
 end;
...
procedure TMyDBLookupComboBox.ListLinkDataChanged;
begin
